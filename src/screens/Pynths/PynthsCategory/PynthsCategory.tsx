...
const PynthsCategory = ({isActive, setIsActive}) => {
    ...
    const getCount = () => {
        ...
        pynthsCategories.forEach(category => {
            if(category === 'all') {
                ...
                value.push(pynths[dexNetworkId].filter(e => e.categories.includes(category)).length)
            }
        })
    ...
    return <>
            <div className="lg:hidden">
                <Flicking className="h-15 bg-navy-500 mb-5 text-lg font-light p-4" plugins={[new Arrow()]} moveType="freeScroll" bound={true}>
                    {pynthsCategories.map((e, index) => 
                        <span className={`button ${index === activeCategory ? 'font-medium text-blue-500' : 'font-medium'} outline-none px-2`} key={index} onClick={()=>setActiveCategory(index)}>{e.toUpperCase()} ({pynthsCategoriesCount[index]})</span>
                    )}
                    <ViewportSlot>
                        <span className="flicking-arrow-prev w-8"><img src="/images/icon/left-arrow.svg"/></span>
                        <span className="flicking-arrow-next w-8"><img src="/images/icon/right-arrow.svg"/></span>
                    </ViewportSlot>
                </Flicking>
            </div>
