  ...
  const init = () => {
    ...
    const networkDebts = getSupportedNetworks().map((e, index) => {
      ...
      return {
        ...
        per: formatNumberToPer(
          networkCachedDebts[e.toString()].total,
          networkCachedDebts.total.total
        ),
      };
    });
    ...
    setNetworkDebts(
      networkDebts.sort((a, b) => Number(b.totalDebt) - Number(a.totalDebt))
    );
  };
            ...
            <div className="flex space-x-5 my-4 items-center">
              <div className="w-40 lg:w-52 h-40 lg:h-52">
                <PieChart
                  ...
                  y={"per"}
                  data={networkDebts}
                  colors={getColors(networkDebts)}
                ></PieChart>
              </div>
              <div className="space-y-3 lg:space-y-5 self-center flex-1">
                <div>
                  <div className="text-3xl lg:text-4xl text-blue-500 font-medium">
                    {formatShortenCurrency(totalDebts)}
                  </div>
