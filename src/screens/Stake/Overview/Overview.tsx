...
const Overview = () => {
  ...
  const { circulatingSupply } = useSelector(
    (state: RootState) => state.circulatingSupply
  );
  ...
  const getMarketCap = () => {
    let tmp = BigInt(
      BigInt(Number(lastRate.price) * 10 ** 18) * circulatingSupply
    );
      ...
      <div className="flex mt-4 lg:gap-5">
        ...
        <div className="flex flex-col w-1/2 lg:w-max pl-5">
          ...
          <div className="flex flex-col lg:flex-row lg:gap-5 lg:items-end">
            <div className="text-xl font-medium text-blue-500">
              ${lastRate.formatPrice}
            </div>
            <div
              className={`flex w-20 h-6 rounded-xl ${
                Number(per) > 0
                  ? "text-blue-800 bg-blue-900"
                  : "text-red-800 bg-red-900"
