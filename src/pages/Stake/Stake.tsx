...
import { getStableDebt, getChartRates, getDebtCaches, getPeriholderCounts } from 'lib/thegraph/api'
...
const Stake = () => {
    ...
    const getDebts = () => {
        ...
        const debts = Promise.all(promise).then((data) => {
            ...
            Object.keys(data[0]).forEach(e => {
                value[e] = getPERIandStalbeDebt(
                    Object.assign(data[0][e], data[1][e])
                );
            });
            return value;
        });
    ...
    const init = async () => {
        const [debt, apy, chartRate, circulatingSupply, networkByDebtCashes, periholderCounts] = await Promise.all([
            getDebts(),
            getTotalAPY(),
            getChartRates(
                {
