const ColorVerticalLabel = ({ color, text, per, size = "lg" }) => {
	return (
		<div className={`flex flex-col justify-end text-gray-700 w-26 bg-600 ${size === "sm" ? "justify-center lg:w-38" : "justify-start lg:w-27"}`}>
			...
			<div className={`flex ${size === "sm" ? "flex-row" : "flex-row"} items-start gap-[2px] xs:gap-1`}>
				<div className={`${color} rounded-sm ${size === "sm" ? "lg:w-4 lg:h-4" : " w-2 h-2 xs:w-3 xs:h-3 ss:w-4 ss:h-4  sm:w-5 sm:h-5"} mx-1 my-1 lg:my-0`}/>
